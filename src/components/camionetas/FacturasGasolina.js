...
export const  FacturasGasolina = ({camionetaInfo,socket}) => {
    ...
    const handleSearch = (value) =>{
        ...
        setInformacionFacturas({total,numeroRegistros});
        return setFacturasGasolina(resultadosBusqueda);
    }
    ...
    const handleUpload = async () =>{
        ...
        let body;
        try {
            const resp = await fetchConTokenSinJSON(`/uploads/camionetas/camioneta/${camionetaId}/gasolina`,formData,"POST");
            body = await resp.json();
            if(resp.status === 200){
                message.success("Subida con exito!");
                //setFacturasGasolina(state => [...state,nuevaFactura]);
                socket.emit("camioneta-actualizada",camionetaId);
            }else{
    ...
    if((informacionFacturas && facturasGasolina) === undefined){
        return <Loading/>
    }else{
        return (
            <>
                ...
                {/*Buscador con autocompletado*/}
                    ...
                    <Table columns={columns} dataSource={facturasGasolina} bordered className="mt-3"/>
                    <Modal title="Agregar factura" visible={isModalVisible} onOk={()=>{setIsModalVisible(false)}} onCancel={()=>{setIsModalVisible(false)}} footer={null}>
                            <h1>Subir factura de gasolina</h1>
