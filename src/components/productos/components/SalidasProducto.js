...
export const SalidasProducto = ({registros}) => {
    ...
    return (
        <>
            ...
            />
            <Table columns={columns} dataSource={registrosSalidas} className="mt-3" size="large"/>
            ...
            {informacionRetiro != null && (
                <Drawer width={640} placement="right" closable={false} onClose={()=>{setIsDrawerVisible(false);setInformacionRetiro(null)}} visible={isDrawerVisible}>
                    ...
                    <Divider/>
                    <p className="site-description-item-profile-p">Documento del retiro</p>
                    <Button type="primary">Descargar documento</Button>
                </Drawer>
            )}
        </>
    )
