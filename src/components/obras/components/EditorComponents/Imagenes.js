...
export const Imagenes = ({obraInfo,socket}) => {
    ...
    const props = {
      ...
      onChange(info) {
        ...
        if (status === 'done') {
          message.success(`El archivo con nombre "${info.file.name}" se ha subido con exito con la categoria de ${categoria} al servidor!`);
          //TODO : Mandar la señal al socket del server
          socket.emit("actualizar-imagenes",{obraId});
        } else if (status === 'error') {
          message.error(`${info.file.name} Error en la subidad de archivos!`);
        }
      },
      onDrop(e) {
      ...
      const onRemove = async (values) =>{
        const { name } = values;
        try {
            const resp = await fetchConToken(`/uploads/obras/obra/${obraId}/${name}`,undefined,"DELETE");
      ...
      return (
          ...
          <div className="p-lg-2">
            <div className="text-center">
              ...
              <p className="lead">Sección para la subidad de imagenes acerca de la obra como trabajos,resultados,etc.</p>
              <Divider/>
              ...
              <Button type="primary" loading={isLoading} onClick={showModal} icon={<CaretUpOutlined/>} size="large">Agregar una nueva imagen!</Button>
            </div>
          ...
          <Modal visible={isModalVisible} onOk={handleOk} footer={null} onCancel={handleCancel}>
            <h1>Agregar una nueva imagen!</h1>
            <p className="lead">Selecciona la nueva imagen que quieras agregas a la galeria de la obra y despues agrega la categoria a la que esta pertenece.</p>
            
            <div className="form-group">
            <label>Categoria de la imagen</label>

            <select defaultValue={categoria} className="form-control mt-2" value={categoria} onChange={handleChangeCategoria}>
              <option>Todas</option>
            ...
            <Dragger {...props} action={`http://localhost:4000/api/uploads/obras/obra/${obraId}`} method="PUT" data={formatearArchivo} style={{width:"100%"}} className="mx-auto mt-5">
                  <p className="ant-upload-drag-icon">
                      <InboxOutlined />
                  </p>
                  <p className="ant-upload-text">Click o arrastra tus imagenes!</p>
                  <p className="ant-upload-hint">
                      Soporte para una o varias imagenes con extension ".PNG" ".JPG" ".GIF" 
                  </p>
            </Dragger>
