...
export const ObraScreen = () => {
    ...
    return (
        <div
            className="container mt-lg-5 p-5 shadow rounded"
            style={{ height: "100vh", overflowY: "auto" }}
        >
            <h1>{`${obraInfo.titulo}`}</h1>
            <span className="lead">
                Pulsa en los botones para navegar en cada etapa de la obra /
                servicio.
            </span>

            <Steps current={obraInfo.estadoReporte} className="mt-5">
                {steps.map((item) => (
                    <Step key={item.title} title={item.title} />
                ))}
            </Steps>

            <div className="steps-content mt-sm-2 mt-md-4">
            ...
            <div className="steps-action">
                ...
                {current === steps.length - 1 && (
                    <Button
                        type="primary"
                        onClick={() =>
                            message.success("La obra ha finalizado!")
                        }
                    >
                        Finalizado
                    </Button>
