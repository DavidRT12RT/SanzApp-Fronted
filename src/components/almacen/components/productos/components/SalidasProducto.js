...
export const SalidasProducto = ({ registros, informacionProducto }) => {
    ...
    const filtrarSalidas = async (values) => {
        ...
        salidasFiltradas = registrosSalidas.filter((salida) => {
            if (
                ...
                if (
                    salida.tipo === "obra" &&
                    values.beneficiarioObra.includes(salida.beneficiario._id)
                )
                    return salida;
                if (
                    salida.tipo === "resguardo" &&
                    values.beneficiarioResguardo.includes(
                        salida.beneficiario._id
                    )
                )
                    return salida;
            }
    ...
    const columns = [
        {
            ...
            render: (text, record) => {
                switch (text) {
                    case "obra":
                        return <Tag color="green">OBRA</Tag>;
                    case "resguardo":
                        return <Tag color="yellow">RESGUARDO</Tag>;
                    case "merma":
                        return <Tag color="red">MERMA</Tag>;
                }
            },
        },
        {
            ...
            render: (text, record) => {
                switch (record.tipo) {
                    case "obra":
                        return <span>{record.beneficiario.titulo}</span>;

                    case "resguardo":
                        return <span>{record.beneficiario.nombre}</span>;

                    case "merma":
                        return <span>Nadie</span>;
                }
            },
        },
        {
    ...
    const renderizarInformacionBeneficiario = () => {
        switch (informacionRetiro.tipo) {
                ...
                return (
                    <>
                        <Row>
                            <Col span={12}>
                                <DescriptionItem
                                    ...
                                    content={
                                        informacionRetiro.beneficiario.titulo
                                    }
                                />
                            </Col>
                        </Row>
                        <Row>
                            <Col span={12}>
                                <DescriptionItem
                                    title="Tipo de reporte de la obra"
                                    content={
                                        informacionRetiro.beneficiario
                                            .tipoReporte
                                    }
                                />
                            </Col>
                        ...
                        <Row>
                            <Col span={12}>
                                <DescriptionItem
                                    ...
                                    content={
                                        informacionRetiro.beneficiario.nombre
                                    }
                                />
                            </Col>
                            <Col span={12}>
                                <DescriptionItem
                                    ...
                                    content={
                                        informacionRetiro.beneficiario.correo
                                    }
                                />
                            </Col>
                        </Row>
            ...
            case "merma":
                return (
                    <p>
                        Al ser merma no hay ninguna persona o obra que reciba el
                        producto
                    </p>
                );
        }
    ...
    if (isLoadingObras || isLoadingEmpleados) return <SanzSpinner />;
    else
        return (
            <>
                <div className="d-flex justify-content-start align-items-center flex-wrap gap-2 mb-3">
                    {isSearching ? (
                        <Button type="primary" danger onClick={limpiarFiltros}>
                            Borrar filtros
                        </Button>
                    ) : (
                        <Button
                    ...
                    {location.pathname.startsWith("/almacen") && (
                        <Button
                            type="primary"
                            onClick={() => {
                                setIsModalVisibleReporte(true);
                            }}
                        >
                            Crear reporte de salidas
                        </Button>
                    )}
                </div>
                <Table
                    columns={columns}
                    dataSource={registrosSalidas}
                ...
                {informacionRetiro != null && (
                    ...
                    >
                        ...
                        >
                            Informacion detallada del retiro
                        </p>
                        <p className="site-description-item-profile-p">
                            Informacion sobre el beneficiario
                        </p>
                        ...
                        <Row>
                            <Col span={12}>
                                <DescriptionItem
                                    title="Fecha del retiro"
                                    content={informacionRetiro.fecha}
                                />
                            </Col>
                            <Col span={12}>
                                <DescriptionItem
                                    title="Cantidad retirada del producto"
                                    content={informacionRetiro.cantidadRetirada}
                                />
                            </Col>
                        </Row>
                        <Row>
                            <Col span={24}>
                                <DescriptionItem
                                    ...
                                    content={informacionRetiro.motivo}
                                />
                            </Col>
                        </Row>
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-muted">
                            (NO seleccionar ningun beneficiario en alguna
                            categoria regresara todos los registros de esa
                            categoria)
                        </p>
                ...
                >
                    <Form
                        onFinish={(values) => {
                            values.reporte = true;
                            filtrarSalidas(values);
                        }}
                        layout="vertical"
                        form={form}
                    >
                        <h1 className="titulo" style={{ fontSize: "30px" }}>
                            Filtrar los registros del reporte
                        </h1>
                        <p className="descripcion">
                            Marca en las siguientes casillas que informacion
                            quieras que contengan el reporte de salidas del
                            producto
                        </p>
                        <Form.Item
                            label="Intervalo de fecha"
