...
export const InventariosAlmacen = () => {
    ...
    const tiposDeInventarios = ["TODOS","VARIOS-PRODUCTOS","UN-PRODUCTO","POR-CATEGORIA"];
        ...
        return (
            <>
                <div className="containerInventarios">
                    ...
                    <Divider/>
                    <h1 className="titulo-descripcion" style={{fontSize:"13px"}}>Tipo de inventario</h1>
                    <div className="containerCategorias mt-3">
                        {
                            tiposDeInventarios.map(categoria => (
                                <CategoriaCard categoria={categoria} categorias={categorias} setCategorias={setCategorias}/>
                            ))
                        }

                    </div>
                    <h1 className="titulo-descripcion mt-5" style={{fontSize:"20px"}}>INVENTARIOS ENCONTRADOS</h1>
                    <Divider/>
                    <div className="containerInventariosRegistros">
                        {
                            registrosInventarios.map(inventario => (
                                <InventarioCard inventario={inventario} key={inventario.key}/>
                            ))
                        }
                    </div>
                </div>
