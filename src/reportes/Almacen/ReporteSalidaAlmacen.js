        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.salidaInformacionContainer}>
                    <Text style={styles.informacionSalida}>Informacion de la salida:</Text>
                    <Text style={styles.label}>Costo TOTAL de salida: <Text style={{...styles.label,color:"green"}}>${salida.costoTotal}</Text></Text>
                    <Text style={styles.label}>Tipo de salida: <Text style={{...styles.label,color:"red"}}>{salida.tipo.toUpperCase()}</Text></Text>
                    <Text style={styles.label}>Fecha de registro: {salida.fechaCreacion}</Text>
                    <Text style={styles.label}>Ultima vez actualizada {salida.fechaUltimaActualizacion}</Text>
                    <Text style={styles.label}>Numero de productos retirados del almacen: {salida.listaProductos.length}</Text>
                    <Text style={styles.label}>Numero de productos devueltos al almacen: {salida.productosDevueltos.length}</Text>
                </View>

                <View style={styles.salidaContainer}>
                    <Text style={{...styles.label,color:"red"}}>Productos retirados del almacen</Text>
                    <Table
                        th
                        col={['16.666%','16.666%','16.666%','16.666%','16.666%','16.666%',]}
                        children={[
                            ["Nombre","Marca","Cantidad retirada","Unidad","Costo X unidad","Costo total del producto"],
                            ...salida.listaProductos.map(producto=> {
                                return [producto.id.nombre,producto.id.marca,producto.cantidad,producto.id.unidad,`$${producto.costoXunidad}`,`$${(producto.cantidad * producto.costoXunidad)}`]
                            })
                        ]} 
                    />
                </View>
                ...
                {salida.productosDevueltos.map(entrada => (
                    <View style={styles.entradaContainer} key={entrada.key}>
                        <Text style={{...styles.label,color:"red"}}>Productos devueltos a almacen</Text>
