    ...
    salidas = salidas.map((salida) => {
        if (salida.tipo === "obra")
            return [
                ...
                salida.cantidadRetirada,
                salida.fecha,
                salida.beneficiario.titulo,
                salida.motivo,
            ];
        if (salida.tipo === "resguardo")
            return [
                salida.tipo.toUpperCase(),
                salida.cantidadRetirada,
                salida.fecha,
                salida.beneficiario.nombre,
                salida.motivo,
            ];
    });
        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <Text style={styles.reportTitle}>
                    Reporte salidas del producto
                </Text>
                <Text style={styles.descripcion}>
                    Salidas que ha tenido el producto dentro de almacen.
                </Text>

                <View style={styles.headerContainer}>
                    <Text style={styles.informacionProducto}>
                        Informacion del producto:
                    </Text>
                    <Text style={styles.label}>
                        Nombre del producto: {informacionProducto.nombre}
                    </Text>
                    <Text style={styles.label}>
                ...
                <View style={styles.dateSelectContainer}>
                    <Text style={styles.dateSelect}>
                        Intervalo de fecha de los registros
                    </Text>
                    <Text style={styles.label}>
                        {intervaloFecha[0]} --- {intervaloFecha[1]}
                    </Text>
                </View>

                <Text style={styles.titleSalidas}>Salidas</Text>
                <Text style={styles.categoriasFiltradas}>
                    (Filtrando salidas por las categorias:{" "}
                    {salidasCategorias.join()})
                </Text>
                <Table
                    style={styles.table}
                    th
